...
import Document.MyHome.DiskPanel;
...
public class ContentPanel extends JPanel
{
	...
	ContentPanel(ContentPanel father)
	{
		...
		folderList=new Vector<Folder>();
		fileList=new Vector<MyText>();
	...
	public void refresh()
	{
		...
		if (folder==null){
			MyToolBar.getToolBar().setAddress("我的磁盘/");
		}
		else{
			MyToolBar.getToolBar().setAddress(folder.getAddress());
		}
	}
	
	public Folder getFolder()
	{
		if (this==MyHome.contentPanel) return null;
	...
	{	
		...
		if (fatherFolder==null) return MyHome.block[0];
		else return fatherFolder.block;
	}
	
	public void createFolder()
	{
		Block block=MyHome.fat.getBlock();
		if (block==null)
		{
	...
	static void switchPanel(ContentPanel contentPanel)
	{
		...
		MyHome.getMainPanel().removeAll();
		MyHome.getMainPanel().add(contentPanel);
	...
	static void switchPanel(DiskPanel panel)
	{
		runningPanel=null;
		MyHome.getMainPanel().removeAll();
		MyHome.getMainPanel().add(panel);
		MyHome.getMainPanel().repaint();
		MyHome.getMainPanel().updateUI();
	}
}
