...
public class Folder extends FCB
{
	...
	public Folder(Block block,ContentPanel father)
	{
		...
		while (true)		//文件夹
		{
			...
			new Folder(MyHome.block[blockIndex],this.contentPanel);
			str=str.substring(index+1);
		}
		while (!str.isEmpty())
		{
			int index=str.indexOf('\n');
			int blockIndex=new Integer(str.substring(0,index)).intValue();
			new MyText(MyHome.block[blockIndex],this.contentPanel);
			str=str.substring(index+1);
		}
	...
	public boolean delete(boolean isRootPanel)
	{
		...
		if (isDelete)
		{
			...
			Block fatherBlock;
			if (fatherContentPanel.getFolder()==null) fatherBlock=MyHome.block[0];
	...
	void resetName(boolean isAutomic)	
	{
		...
		if (!checkName(newName))
		{
			...
			{
				JOptionPane.showMessageDialog(null, "现已修复重命名", "重命名出错", JOptionPane.YES_OPTION, null);
			}
